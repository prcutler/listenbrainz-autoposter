...
- A scheduled GitHub Actions workflow runs **every Tuesday at 15:07 UTC**.
...
### Why Tuesday / why "last week"?

ListenBrainz's `week` range is **last week's completed Monday–Sunday**, *not* a rolling 7 days.
Its stats are also batch-computed with a day or two of lag. Running on **Tuesday** guarantees last
week's numbers are final before posting.
...
without dry run (or just wait for Tuesday).
...
```
.github/workflows/weekly-post.yml   # the Tuesday schedule + manual trigger
